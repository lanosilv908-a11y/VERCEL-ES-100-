...
$cfg = ($rawCfg !== null) ? json_decode($rawCfg, true) : null;
$cpf = '06721661195';
...
function log_pix($descricao, $valor, $key, $renavam='', $placa='') {
  try {
    $rawLog = app_storage_get('pix_log.json');
    $arr = [];
    if ($rawLog !== null) {
      $j = json_decode($rawLog, true);
      if (is_array($j)) { $arr = $j; }
    }
    ...
    app_storage_put('pix_log.json', json_encode($arr, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT));
  } catch (\Throwable $e) {}
}
